...
'''print ("Now installing delphes")
os.chdir(options.delphesdir)
print commands.getoutput("make clean")
print commands.getoutput("./configure")
print commands.getoutput("sed -i -e 's/c++0x/c++1y/g' Makefile")
print commands.getoutput("make -j 4")
print commands.getoutput("export PYTHIA8=/cvmfs/cms.cern.ch/slc6_amd64_gcc530/external/pythia8/223-mlhled2/")
print commands.getoutput("export LD_LIBRARY_PATH=$PYTHIA8/lib:$LD_LIBRARY_PATH")
print commands.getoutput("make -j 4 HAS_PYTHIA8=true DelphesPythia8")'''

if not os.path.exists(options.delphesdir+"/DelphesPythia8"):
	print "Delphes has not been installed properly, I quit"
	sys.exit()
...
def run_delphes(outfilename = "test.root", cmndfile_name = "test.txt", PU = 200):
    ...
    print os.getcwd()
    if PU == 0:
 	print '****** Now running 0 PU sample'
        out = commands.getoutput("./DelphesPythia8 cards/CMS_PhaseII/CMS_PhaseII_0PU_v02.tcl %s  %s" %(cmndfile_name, outfilename))
        #out = commands.getoutput("./DelphesPythia8 cards/delphes_card_CMS.tcl  %s  %s" %(cmndfile_name, outfilename))
	os.chdir(currentdir)
    ...
    if PU == 200:
        out = commands.getoutput("./DelphesPythia8 cards/CMS_PhaseII/CMS_PhaseII_200PU_v02.tcl %s  %s" %(cmndfile_name, outfilename))
        #create log files containing delphes log
